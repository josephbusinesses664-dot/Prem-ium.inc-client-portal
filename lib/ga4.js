...
function getClient() {
  ...
  if (!raw) {
    console.error('[ga4] GOOGLE_CREDENTIALS env var is not set');
    return null;
  }
  if (!PROPERTY_ID) {
    console.error('[ga4] GA4_PROPERTY_ID env var is not set');
    return null;
  }
  try {
    const credentials = JSON.parse(raw);
    console.log('[ga4] client init — project:', credentials.project_id, '| property:', PROPERTY_ID);
    return new BetaAnalyticsDataClient({ credentials });
  } catch (e) {
    console.error('[ga4] GOOGLE_CREDENTIALS is not valid JSON:', e.message);
    return null;
  }
}
...
async function getSiteStats(hostname, dateRange = '30daysAgo') {
  console.log('[ga4] getSiteStats hostname=%s period=%s', hostname, dateRange);
  try {
    // KPI totals
