...
async function getSiteHTML(repoName) {
  const { content, sha } = await getFile(repoName, 'index.html');
  return { html: content, sha };
}

async function putSiteHTML(repoName, html, sha, commitMsg) {
  return await putFile(repoName, 'index.html', html, sha, commitMsg);
}
...
const SYNC_EXCLUDE = new Set([PORTAL_REPO, 'prem-ium-inc']);
...
async function syncSitesFromGitHub() {
  ...
  for (const repo of repos) {
    ...
    // Repos without a real description would register under their random slug — skip them
    if (!business || business.toLowerCase() === slug.toLowerCase()) continue;
...
module.exports = { readData, writeData, getSiteHTML, putSiteHTML, putSiteImage, injectGaTag, getFile, putFile, bootstrap, syncSitesFromGitHub };
