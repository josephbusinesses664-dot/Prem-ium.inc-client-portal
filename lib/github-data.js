...
// Fetch any file from a site repo as raw bytes + a content-type, for serving
// the site directly from the portal (assets: css/js/images/sitemap/etc.).
const RAW_MIME = {
...
async function getRepoFileRaw(repoName, filePath) {
  const url = `${API}/repos/${GH_ORG}/${repoName}/contents/${filePath}`;
  const r = await fetch(url, { headers: headers() });
  if (!r.ok) return null;
  const data = await r.json();
  ...
  if (data.content) {
    ...
    // Files >1MB come back with empty content — pull the raw bytes directly.
    const dr = await fetch(data.download_url);
    if (!dr.ok) return null;
    buffer = Buffer.from(await dr.arrayBuffer());
  } else {
    return null;
  }
  const ext = (filePath.split('.').pop() || '').toLowerCase();
  return { buffer, contentType: RAW_MIME[ext] || 'application/octet-stream' };
}
...
// Turn a business name into a clean URL slug: "Redline Barbershop" → "redline-barbershop"
function slugify(s) {
  return String(s || '')
    .normalize('NFKD').replace(/[̀-ͯ]/g, '') // strip accents
    .toLowerCase()
    .replace(/&/g, ' and ')
    .replace(/['’]/g, '') // drop apostrophes so "Fowler's" → "fowlers"
    .replace(/[^a-z0-9]+/g, '-')
    .replace(/^-+|-+$/g, '')
    .slice(0, 60);
}

// Pick a nameSlug not already used: fall back to appending the city, then a counter.
function uniqueNameSlug(base, taken, city) {
  if (!taken.has(base)) return base;
  const withCity = `${base}-${slugify(city)}`;
  if (slugify(city) && !taken.has(withCity)) return withCity;
  let i = 2;
  while (taken.has(`${base}-${i}`)) i++;
  return `${base}-${i}`;
}

// Assign a stable, unique nameSlug to every record missing one. The random `slug`
// key stays canonical (/live/:slug never breaks); nameSlug powers the pretty
// /s/:nameSlug brand links. Returns true if any record changed.
function ensureNameSlugs(sites) {
  const taken = new Set(Object.values(sites).map(s => s.nameSlug).filter(Boolean));
  let changed = false;
  for (const s of Object.values(sites)) {
    if (s.nameSlug) continue;
    const base = slugify(s.business);
    if (!base) continue; // no business name → no pretty link; /live/:slug still works
    const ns = uniqueNameSlug(base, taken, s.city);
    s.nameSlug = ns;
    taken.add(ns);
    changed = true;
  }
...
async function syncSitesFromGitHub() {
  ...
  for (const repo of repos) {
    ...
    sites[slug] = {
      ...
      city: '',
      category: 'Business',
      // Fleet deploys the Render service at the bare slug (e.g. royal-bakery-jaipur.onrender.com),
      // NOT the preview-prefixed repo name. Match that so analytics/host lookups are correct.
  ...
  // Backfill/assign pretty nameSlugs for new and existing records.
  const slugsChanged = ensureNameSlugs(sites);

  if (added.length || slugsChanged) {
    const msg = added.length
      ? `Auto-sync: register ${added.length} new site(s) — ${added.join(', ')}`
      : 'Auto-sync: backfill nameSlugs';
    await writeData('sites', sites, msg);
    if (added.length) console.log(`[sync] Registered ${added.length} new site(s): ${added.join(', ')}`);
    if (slugsChanged) console.log('[sync] Updated nameSlugs');
  }
  return { added };
}
...
module.exports = { readData, writeData, getSiteHTML, putSiteHTML, putSiteImage, getRepoFileRaw, listSitePages, listFileCommits, getFileAtRef, injectGaTag, getFile, putFile, bootstrap, syncSitesFromGitHub, slugify, ensureNameSlugs };
